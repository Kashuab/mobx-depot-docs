...
The `DepotGQLClient`, which is responsible for sending GQL requests, has a minimal cache store that does not provide
...
The `UserQuery` is dispatched, and the `cache-and-network` policy is used. The cache is checked, and since it's empty,
the network request is dispatched. The response is received, and the `RootStore` is updated with the new `User` instance.
The `User` instance is also cached in the `DepotGQLClient` cache store.
