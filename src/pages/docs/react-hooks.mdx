...
## Provide arguments to `dispatch`

You can provide arguments to `dispatch` by defining them in the `useMutation` hook:

```tsx
export const TodoList = observer(({ todos }) => {
  const { data, loading, dispatch } = useMutation(
    (id: string) => new DeleteTodoMutation(
      { id },
      todo => todo.title.content,
    ),
    {
    ...
    <div className="TodoList">
      ...
      {todos.map(todo => (
        <div className="Todo">
          <h1>{todo.title}</h1>
          <p>{todo.content}</p>
          <button onClick={() => dispatch(todo.id)}>Delete</button>
        </div>
      ))}
    </div>
  )
